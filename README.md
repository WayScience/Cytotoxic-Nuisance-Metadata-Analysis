...
As a result, we generated two distinct types of feature-selected profiles:

- **JUMP-Aligned Feature-Selected Cell Injury Profiles:** These profiles were created by applying feature selection to the cell injury profiles where the feature space is aligned with the JUMP data.

- **Feature-Selected Cell Injury Profiles:** These profiles were generated by applying feature selection exclusively to the cell injury dataset, without any alignment to the JUMP data feature space.
...
The models also included `class_weight="balanced"` to ensure the model pays more attention to minority classes during training, preventing label imbalance or bias.
